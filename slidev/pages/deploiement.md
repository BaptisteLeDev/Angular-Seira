...
<!--
**[DÉPLOIEMENT]** Cinquième partie : préparer, documenter et mettre en production.
-->
...
<!--
**[PRÉPARER]** Backend conteneurisé, frontend sur Vercel — deux cibles distinctes.

- **Backend** : `docker-compose.yml`, migrations + seeders versionnés, config par `.env`
- **Vercel** : `vercel.json` gère pnpm, build, et le **rewrite SPA** → `index.html`
- Variables `NG_APP_*` définies dans Vercel — bascule locale/prod triviale
- **Documentation** : `CLAUDE.md`, `SCHEMA_BDD.md`, `TODO.md`, contrat OpenAPI auto

→ Trois comptes de seed documentés pour une démo immédiate.
-->
...
<!--
**[DEVOPS]** Flux simple et continu — push → build → déploiement automatique.

- **Push sur une branche** → preview Vercel automatique, prod sur la branche principale
- **Docker** garantit la parité des environnements local ↔ intégration
- Intégration par **Pull Request** : une branche par lot, revue, puis fusion
- Axe d'amélioration assumé : **CI GitHub Actions** pour lancer les tests à chaque PR

→ Présenté comme un axe de progression, pas comme un acquis.
-->
